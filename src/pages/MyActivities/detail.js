...
export default function ActivityDetail(props) {
  ...
  const [startDate, setStartDate] = useState(props.startDate.toISOString().split("T")[0]);
  const [endDate, setEndDate] = useState(props.endDate.toISOString().split("T")[0]);

  return (
    <div className="flex flex-col justify-center mx-10 mb-10">
      <h2 className="text-sgu-blue text-4xl pt-10 pb-20">Update Activity</h2>
      ...
      <div className="relative flex">
        <form className="relative text-2xl pt-5 w-full">
          ...
          <label htmlFor="currentAmount" className="mb-2 font-bold">
            Current Amount:
          </label>
          <input
            className="border-2 ml-3 px-1 mb-5"
          ...
          <button className="bg-sgu-blue text-white rounded-lg p-2 mt-5 mx-3">Update</button>
          <button className="bg-red-600 text-white rounded-lg p-2 mt-5 mx-3">Delete</button>
        </form>
      </div>
    </div>
  );
